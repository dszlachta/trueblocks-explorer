...
const TableCellCheckbox = ({ name, row, selected }) => {
  ...
  const handleOnRowSelected = useCallback(
    () => dispatch({ type: 'SELECT_SINGLE_ROW', row, rows: data, isRowSelected: selected, keyField }),
    [data, selected, row]
  );

  return (
    <TableCellCheckboxStyle onClick={(e) => e.stopPropagation()} className="rdt_TableCell" noPadding>
      <Checkbox
        name={name}
