...
import { accountsSchema } from './AccountsSummarySchema';
...
//---------------------------------------------------------------------------
export const AccountsSummary = (props) => {
  var context = useContext(PageContext)
  const { accounts, dispatch } = useAccountsSummary();
  ...
  useEffect(() => {
    ...
    if (partialFetch) {
      ...
      refreshAccountsSummaryData2(dataQuery, dispatch, mocked, 0, max_records, recordCount, statusDispatch);
    }
    // EXISTING_CODE
    if (!partialFetch) {
      refreshAccountsSummaryData(dataQuery, dispatch, mocked);
    }
...
//----------------------------------------------------------------------
export function refreshAccountsSummaryData2(query, dispatch, mocked, firstRecord, maxRecords, nRecords, statusDispatch) {
  if (mocked) return refreshAccountsSummaryData(query, dispatch, mocked);
  getServerData(
    getDataUrl(),
    ...
    if (maxRecords < nRecords) {
      statusDispatch(LOADING);
      refreshAccountsSummaryData2(query, dispatch, mocked, 0, maxRecords * 2, nRecords, statusDispatch);
    }
  });
}
// EXISTING_CODE
...
//----------------------------------------------------------------------
export function refreshAccountsSummaryData(query, dispatch, mocked) {
